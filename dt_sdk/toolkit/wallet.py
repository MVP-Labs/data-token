...
from dt_sdk.toolkit.constants import MIN_GAS_PRICE, ENV_MAX_GAS_PRICE
from dt_sdk.toolkit.utils import privateKeyToAddress, privateKeyToPublicKey
...
class Wallet:
    ...
    def __init__(self, web3,
        ...
        if encrypted_key and not private_key:
            ...
            self._key = self._web3.eth.account.decrypt(
                encrypted_key, self._password)
    ...
    @property
    def atp_address(self):
        # if 'atp' in self._address:
        _, data = bech32.decode("atp", self._address)
        return to_checksum_address(bytes(data))
    ...
    def _get_nonce(web3, address):
        ...
        if address not in Wallet._last_tx_count:
            Wallet._last_tx_count[address] = web3.eth.getTransactionCount(
                address)
        else:
